...
)                        /* Move to zero only writes back dirty window */
{
    ...
    if (wsect != sector) {    /* Changed current window */
        ...
        if (fs->winflag) {    /* Write back dirty window if needed */
            if (eDisk_Write(fs->drive, fs->win, wsect, 1) != RES_OK)
                return FALSE;
            fs->winflag = 0;
            if (wsect < (fs->fatbase + fs->sects_fat)) {    /* In FAT area */
                for (n = fs->n_fats; n >= 2; n--) {    /* Refrect the change to FAT copy */
                    wsect += fs->sects_fat;
                    eDisk_Write(fs->drive, fs->win, wsect, 1);
                }
            }
        }
#endif
        if (sector) {
            if (eDisk_Read(fs->drive, fs->win, sector, 1) != RES_OK)
                return FALSE;
            fs->winsect = sector;
        }
    }
...
)
{
    ...
    if (fs->fs_type == FS_FAT32 && fs->fsi_flag) {        /* Update FSInfo sector if needed */
        ...
        eDisk_Write(0, fs->win, fs->fsi_sector, 1);
        fs->fsi_flag = 0;
    }
#endif
...
)
{
    ...
    for (n = fs->sects_clust; n; n--) {
        if (eDisk_Write(fs->drive, fs->win, sector, 1) != RES_OK)
            return FR_RW_ERROR;
        sector++;
    }
    fs->winflag = 1;
...
{
    if (eDisk_Read(fs->drive, fs->win, sect, 1) != RES_OK)    /* Load boot record */
        return 2;
...
)
{
    ...
    *rfs = fs;            /* Returen pointer to the corresponding file system object */
    ...
    stat = eDisk_Init(fs->drive);    /* Initialize low level disk I/O layer */
    ...
    if (fmt == FS_FAT32) {
        ...
        if (eDisk_Read(0, fs->win, fs->fsi_sector, 1) == RES_OK &&
...
)
{
    ...
    if (eDisk_Status(fs->drive) & STA_NOINIT)
        return FR_NOT_READY;
    ...
    for ( ;  btr;                                    /* Repeat until all data transferred */
        ...
        if ((fp->fptr & (S_SIZ - 1)) == 0) {        /* On the sector boundary */
            ...
            if (fp->flag & FA__DIRTY) {                /* Flush file I/O buffer if needed */
                if (eDisk_Write(fs->drive, fp->buffer, fp->curr_sect, 1) != RES_OK)
                    goto fr_error;
                fp->flag &= ~FA__DIRTY;
            }
#endif
            ...
            if (cc) {                                /* Read maximum contiguous sectors directly */
                ...
                if (eDisk_Read(fs->drive, rbuff, sect, cc) != RES_OK)
                    goto fr_error;
                fp->sect_clust -= cc - 1;
                fp->curr_sect += cc - 1;
                rcnt = cc * S_SIZ; continue;
            }
            if (eDisk_Read(fs->drive, fp->buffer, sect, 1) != RES_OK)    /* Load the sector into file I/O buffer */
                goto fr_error;
        }
    ...
    for ( ;  btw;                                    /* Repeat until all data transferred */
        ...
        if ((fp->fptr & (S_SIZ - 1)) == 0) {        /* On the sector boundary */
            ...
            if (cc) {                                /* Write maximum contiguous sectors directly */
                if (cc > fp->sect_clust) cc = fp->sect_clust;
                if (eDisk_Write(fs->drive, wbuff, sect, cc) != RES_OK)
                    goto fw_error;
                fp->sect_clust -= cc - 1;
                fp->curr_sect += cc - 1;
            ...
            if (fp->fptr < fp->fsize &&              /* Fill sector buffer with file data if needed */
                eDisk_Read(fs->drive, fp->buffer, sect, 1) != RES_OK)
                    goto fw_error;
        }
    ...
    if (ofs) {
        ...
        if (clust) {            /* If the file has a cluster chain, it can be followed */
            ...
            if ((ofs & (S_SIZ - 1)) &&                    /* Load current sector if needed */
                eDisk_Read(fs->drive, fp->buffer, fp->curr_sect, 1) != RES_OK)
                goto fk_error;
...
)
{
    ...
    for (n = 1; n < fs->sects_clust; n++) {
        if (eDisk_Write(fs->drive, fw, ++dsect, 1) != RES_OK)
            return FR_RW_ERROR;
    }
...
)
{
    ...
    if (!partition) {
        ...
        if (eDisk_Write(drv, fs->win, 0, 1) != RES_OK)
            return FR_RW_ERROR;
    }
    ...
    if (eDisk_Write(drv, tbl, b_part+0, 1) != RES_OK)
        return FR_RW_ERROR;
    if (fmt == FS_FAT32)
        eDisk_Write(drv, tbl, b_part+6, 1);
    ...
    for (m = 0; m < N_FATS; m++) {
        ...
        if (eDisk_Write(drv, tbl, b_fat++, 1) != RES_OK)
            return FR_RW_ERROR;
    ...
    if (fmt == FS_FAT32) {
        ...
        eDisk_Write(drv, tbl, b_part+1, 1);
        eDisk_Write(drv, tbl, b_part+7, 1);
    }
