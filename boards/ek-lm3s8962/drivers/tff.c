...
)
{
	...
	for (n = fs->sects_clust; n; n--) {
		if (eDisk_Write(0, fs->win, sector, 1) != RES_OK)
			return FR_RW_ERROR;
		sector++;
	}
	fs->winflag = 1;
...
)
{
	...
	if (eDisk_Read(0, fs->win, sect, 1) != RES_OK)	/* Load boot record */
		return 2;
...
)
{
	...
	*path = p;				/* Return pointer to the path name */
	...
	stat = eDisk_Init(0);			/* Initialize low level disk I/O layer */
	...
	for ( ;  btr;									/* Repeat until all data transferred */
		...
		if ((fp->fptr % 512) == 0) {				/* On the sector boundary */
			...
			if (cc) {								/* Read maximum contiguous sectors directly */
				...
				if (eDisk_Read(0, rbuff, sect, cc) != RES_OK)
					goto fr_error;
				fp->sect_clust -= cc - 1;
				fp->curr_sect += cc - 1;
	...
	for ( ;  btw;									/* Repeat until all data transferred */
		...
		if ((fp->fptr % 512) == 0) {				/* On the sector boundary */
			...
			if (cc) {								/* Write maximum contiguous sectors directly */
				if (cc > fp->sect_clust) cc = fp->sect_clust;
				if (eDisk_Write(0, wbuff, sect, cc) != RES_OK)
					goto fw_error;
				fp->sect_clust -= cc - 1;
				fp->curr_sect += cc - 1;
...
)
{
	...
	for (n = 1; n < fs->sects_clust; n++) {
		if (eDisk_Write(0, fw, ++dsect, 1) != RES_OK)
			return FR_RW_ERROR;
	}
